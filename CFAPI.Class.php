...
class CFAPI
{
  ...
  *****/
  ...
  public function zone_ips($domain, $hours = 24, $class = 't', $geo = FALSE)
  {
    ...
    if (isset($class)) { $filter['class'] = $class; }
	if (isset($geo))   { $filter['geo'] = 1; }
	return $this->APIQuery($filter);
  }
  ...
  *****/
  ...
  public function rec_new($domain, $type, $name, $content, $ttl = 300, $service_mode = 1,
						  $prio = 0, $service = '', $srvname = '', $protocol = '', $weight = '', $port = '', $target = '')
  {
    $filter['a'] = 'rec_new';
	$filter['z'] = $domain;
	$filter['type'] = $type;
    ...
    if (preg_match('/(A|AAAA|CNAME)/', $type)) { $filter['service_mode'] = $service_mode; }
    if (preg_match('/(MX|SRV)/', $type)) { $filter['prio'] = $prio; }	
  ...
  public function rec_edit($domain, $type, $dns_id, $name, $content, $ttl = 300, $service_mode = 1,
						  $prio = 0, $service = '', $srvname = '', $protocol = '', $weight = '', $port = '', $target = '')
  {
    $filter['a'] = 'rec_edit';
	$filter['z'] = $domain;
  ...
  *****/
  ...
  {
    try { $j = json_decode($in_str); } catch(Exception $e) { return FALSE; }
...
/*
  $tkn = '<TOKEN>';
  $email = '<EMAIL>'
  $cf = new CFAPI($tkn, $email);
  $my_domains = $cf->zone_load_multi();
*/
